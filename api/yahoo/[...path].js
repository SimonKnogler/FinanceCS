// Catch-all Yahoo Finance API proxy for Vercel
module.exports = async (req, res) => {
  ...
  try {
    ...
    const yahooUrl = `https://query1.finance.yahoo.com${extraPath}${queryString}`;

    console.log('Proxying Yahoo request to:', yahooUrl);
    ...
    if (!response.ok) {
      console.error('Yahoo API error:', response.status, yahooUrl);
